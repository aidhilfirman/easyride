      ...
      return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
          <div className="text-center max-w-md animate-fadeIn">
            ...
            <button onClick={() => this.setState({ hasError: false, error: null })} className="rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-medium text-white hover:bg-slate-800 transition-all duration-200 hover:shadow-lg">Try Again</button>
          </div>
        </div>
      );
    }
...
function StatCard({ title, value, hint, accent, icon }) {
  return (
    <div className={"group rounded-2xl border p-4 md:p-5 transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 " + (accent || "border-slate-200/40 bg-white/70 backdrop-blur-sm")}>
      <div className="flex items-center justify-between">
        <div className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">{title}</div>
        {icon && <div className="w-8 h-8 rounded-lg bg-slate-100/80 flex items-center justify-center text-slate-400 group-hover:scale-110 transition-transform duration-300">{icon}</div>}
      </div>
      <div className="mt-2 text-2xl md:text-3xl font-extrabold text-slate-900 tracking-tight">{value}</div>
      {hint && <div className="mt-1.5 text-xs text-slate-400 font-medium">{hint}</div>}
    </div>
  );
}
...
function Avatar({ name, size }) {
  var s = size || "sm";
  var dims = s === "sm" ? "w-7 h-7 text-[10px]" : "w-9 h-9 text-xs";
  var initials = (name || "?").split(" ").map(function (w) { return w[0]; }).join("").toUpperCase().slice(0, 2);
  var colors = ["bg-violet-500", "bg-blue-500", "bg-teal-500", "bg-amber-500", "bg-rose-500", "bg-indigo-500", "bg-emerald-500", "bg-cyan-500"];
  var idx = 0;
  for (var i = 0; i < (name || "").length; i++) idx += (name || "").charCodeAt(i);
  var bg = colors[idx % colors.length];
  return <div className={dims + " rounded-full " + bg + " text-white font-bold flex items-center justify-center shrink-0 ring-2 ring-white"}>{initials}</div>;
}

/* ── Badge ── */
function Badge({ children, tone }) {
  return <span className={"inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-semibold whitespace-nowrap tracking-wide " + tone}>{children}</span>;
}
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 space-y-2 pointer-events-none">
      {items.map(function (t) {
        return (
          <div key={t.id} className={"animate-toastIn rounded-xl px-4 py-3 text-sm font-medium text-white shadow-xl backdrop-blur-sm " + (t.type === "error" ? "bg-red-500/90" : "bg-emerald-500/90")}>
            {t.message}
          </div>
        );
      })}
...
function TicketDrawer({ ticket, onClose, onChange, onSave, onDelete, onAddComment, staffOptions, currentUser, nowMs }) {
  ...
  var fc = "w-full rounded-xl border px-3.5 py-2.5 text-sm outline-none transition-all duration-200 ";
  var fieldClass = fc + (editing ? "border-slate-200 bg-white focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/20 focus:shadow-sm" : "border-slate-100 bg-slate-50/30 text-slate-900 cursor-default");
  ...
  return (
    <div className="fixed inset-0 z-40 flex">
      ...
      <div className="animate-slideIn relative ml-auto flex h-full w-full max-w-2xl flex-col bg-white shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="flex items-start justify-between border-b border-slate-100/80 px-6 py-5 shrink-0 bg-gradient-to-r from-slate-50/50 to-white">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <Badge tone="bg-indigo-500/10 text-indigo-600 border-indigo-200/50">{ticket.id}</Badge>
              <Badge tone={STATUS_STYLES[ticket.status]}>{ticket.status}</Badge>
              {warn === "critical" && <Badge tone="bg-rose-500/10 text-rose-600 border-rose-200/50">SLA Breach</Badge>}
              {warn === "warning" && <Badge tone="bg-amber-500/10 text-amber-600 border-amber-200/50">SLA Warning</Badge>}
            </div>
            <h2 className="mt-3 text-lg font-bold text-slate-900 leading-snug">{ticket.issue || "Untitled ticket"}</h2>
            <div className="mt-1 text-xs text-slate-400 font-medium">{formatDuration(ticket, nowMs)}</div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button type="button" onClick={function () { setEditing(!editing); }} className={"rounded-xl border px-3.5 py-2 text-sm font-medium transition-all duration-200 " + (editing ? "border-indigo-300 bg-indigo-50 text-indigo-600 shadow-sm" : "border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-indigo-300 hover:text-indigo-600")}>
            ...
            <button type="button" onClick={onClose} className="rounded-xl border border-slate-200 p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all duration-200">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <label className="text-sm"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Rider No.</div><input readOnly={ro} value={ticket.riderNo || ""} onChange={(e) => onChange(ticket.id, { riderNo: e.target.value })} className={fieldClass} /></label>
            <label className="text-sm"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Rider Name</div><input readOnly={ro} value={ticket.riderName || ""} onChange={(e) => onChange(ticket.id, { riderName: e.target.value })} className={fieldClass} /></label>
            <label className="text-sm sm:col-span-2"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Issue</div><input readOnly={ro} value={ticket.issue || ""} onChange={(e) => onChange(ticket.id, { issue: e.target.value })} className={fieldClass} /></label>
            <label className="text-sm"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Received</div><input readOnly={ro} value={ticket.timestampReceived || ""} onChange={(e) => onChange(ticket.id, { timestampReceived: e.target.value })} className={fieldClass} /></label>
            <label className="text-sm"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Acknowledged</div><select disabled={ro} value={ticket.acknowledged ? "Done" : ""} onChange={(e) => onChange(ticket.id, { acknowledged: e.target.value === "Done" })} className={fieldClass}><option value="">Not yet</option><option value="Done">Done</option></select></label>
            <label className="text-sm"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Responsible Person</div><input readOnly={ro} list={editing ? "staff-opts" : undefined} value={ticket.responsiblePerson || ""} onChange={(e) => onChange(ticket.id, { responsiblePerson: e.target.value })} className={fieldClass} /></label>
            <label className="text-sm"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Escalated to</div><input readOnly={ro} list={editing ? "staff-opts" : undefined} value={ticket.escalatedTo || ""} onChange={(e) => onChange(ticket.id, { escalatedTo: e.target.value })} className={fieldClass} /></label>
            <label className="text-sm sm:col-span-2"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Solution</div><input readOnly={ro} value={ticket.solution || ""} onChange={(e) => onChange(ticket.id, { solution: e.target.value })} className={fieldClass} /></label>
            <label className="text-sm"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Status</div><select disabled={ro} value={ticket.status} onChange={(e) => onChange(ticket.id, { status: e.target.value })} className={fieldClass}>{STATUS_ORDER.map(function (s) { return <option key={s} value={s}>{s}</option>; })}</select></label>
            <label className="text-sm"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Solved At</div><input readOnly={ro} value={ticket.timestampSolved || ""} onChange={(e) => onChange(ticket.id, { timestampSolved: e.target.value })} className={fieldClass} /></label>
            <label className="text-sm sm:col-span-2"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Duration</div><input readOnly={ro} value={ticket.importedDurationLabel || ""} onChange={(e) => onChange(ticket.id, { importedDurationLabel: e.target.value, duration: e.target.value })} placeholder={editing ? "e.g. 3 days, Same day" : ""} className={fieldClass} /></label>
          </div>

          {/* Internal Notes */}
          <div>
            <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-400">Internal Notes</h4>
            <div className="space-y-3">
              {ticket.comments.length ? ticket.comments.map(function (c) {
                return (
                  <div key={c.id} className="rounded-xl border border-slate-100/80 bg-slate-50/50 p-3 animate-fadeIn">
                    <div className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-2">
                        <Avatar name={c.author} />
                        <div className="text-sm font-semibold text-slate-700">{c.author}</div>
                      </div>
                      <div className="text-xs text-slate-400">{formatDateTime(c.createdAt)}</div>
                    </div>
                    <div className="mt-2 text-sm text-slate-600 leading-relaxed">{c.text}</div>
                  </div>
                );
              ...
              <div className="rounded-xl border border-slate-200/80 p-3">
                <textarea rows={2} value={commentText} onChange={(e) => setCommentText(e.target.value)} placeholder="Add a note..." className={"mb-2 w-full rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-sm outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/20 transition-all duration-200"} />
                <div className="flex justify-end">
                  <button type="button" onClick={function () {
                    ...
                    setCommentText("");
                  }} className="rounded-xl bg-slate-900 px-4 py-2 text-xs font-semibold text-white hover:bg-slate-800 transition-all duration-200 hover:shadow-md">Add Note</button>
                </div>
              </div>
            </div>
          </div>
...
function AckDrawer({ entry, onClose, onChange, onSave, onDelete }) {
  ...
  var fc = "w-full rounded-xl border px-3.5 py-2.5 text-sm outline-none transition-all duration-200 ";
  var fieldClass = fc + (editing ? "border-slate-200 bg-white focus:border-teal-400 focus:ring-2 focus:ring-teal-400/20 focus:shadow-sm" : "border-slate-100 bg-slate-50/30 text-slate-900 cursor-default");
  var ro = !editing;
    ...
    <div className="fixed inset-0 z-40 flex">
      ...
      <div className="animate-slideIn relative ml-auto flex h-full w-full max-w-2xl flex-col bg-white shadow-2xl overflow-hidden">
        <div className="flex items-start justify-between border-b border-slate-100/80 px-6 py-5 shrink-0 bg-gradient-to-r from-slate-50/50 to-white">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <Badge tone="bg-teal-500/10 text-teal-600 border-teal-200/50">{entry.id}</Badge>
            ...
            <h2 className="mt-3 text-lg font-bold text-slate-900 leading-snug">{entry.fullName || entry.shortName || "Unnamed Rider"}</h2>
            <div className="mt-1 text-xs text-slate-400 font-medium">Rider {entry.riderNo}</div>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button type="button" onClick={function () { setEditing(!editing); }} className={"rounded-xl border px-3.5 py-2 text-sm font-medium transition-all duration-200 " + (editing ? "border-teal-300 bg-teal-50 text-teal-600 shadow-sm" : "border-slate-200 text-slate-600 hover:bg-slate-50 hover:border-teal-300 hover:text-teal-600")}>
              <svg className="w-4 h-4 inline-block mr-1.5 -mt-0.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z" /></svg>{editing ? "Editing" : "Edit"}
            </button>
            <button type="button" onClick={onClose} className="rounded-xl border border-slate-200 p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-50 hover:border-slate-300 transition-all duration-200">
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          {/* Rider Info */}
          <div>
            <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-400">Rider Info</h4>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <label className="text-sm"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Rider No.</div><input readOnly={ro} value={entry.riderNo || ""} onChange={(e) => onChange(entry.id, { riderNo: e.target.value })} className={fieldClass} /></label>
              <label className="text-sm"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Short Name</div><input readOnly={ro} value={entry.shortName || ""} onChange={(e) => onChange(entry.id, { shortName: e.target.value })} className={fieldClass} /></label>
              <label className="text-sm"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Full Name</div><input readOnly={ro} value={entry.fullName || ""} onChange={(e) => onChange(entry.id, { fullName: e.target.value })} className={fieldClass} /></label>
            </div>
          </div>
          {/* Confirmation */}
          <div>
            <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-400">Confirmation</h4>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <label className="text-sm"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Confirmation</div><input readOnly={ro} value={entry.confirmation || ""} onChange={(e) => onChange(entry.id, { confirmation: e.target.value, confirmed: e.target.value.toLowerCase() === "yes" || e.target.value.toLowerCase() === "confirmed" || e.target.value.toLowerCase() === "done" })} placeholder={editing ? "e.g. Yes, No, Pending" : ""} className={fieldClass} /></label>
              <label className="text-sm"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Reminder (1 Apr 2026)</div><input readOnly={ro} value={entry.reminderDate || ""} onChange={(e) => onChange(entry.id, { reminderDate: e.target.value })} className={fieldClass} /></label>
            </div>
          </div>
          {/* Proof Reminders */}
          <div>
            <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-400">Proof of Reminders</h4>
              ...
              {[
                ...
                return (
                  <div key={f.key} className="text-sm">
                    <div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">{f.label}</div>
                    {isImage ? (
                      <a href={val} target="_blank" rel="noopener noreferrer">
                        <img src={val} alt={f.label} className="max-w-full max-h-48 rounded-xl border border-slate-200 object-contain cursor-pointer hover:shadow-lg transition-all duration-300" />
                      </a>
                    ) : (
          ...
          {/* Sign Off */}
          <div>
            <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-slate-400">Sign Off</h4>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <label className="text-sm"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Eva Check</div><input readOnly={ro} value={entry.evaCheck || ""} onChange={(e) => onChange(entry.id, { evaCheck: e.target.value })} className={fieldClass} /></label>
              <label className="text-sm"><div className="mb-1.5 font-semibold text-slate-500 text-xs uppercase tracking-wider">Trainer & Checker Sign Off</div><input readOnly={ro} value={entry.trainerSignOff || ""} onChange={(e) => onChange(entry.id, { trainerSignOff: e.target.value })} className={fieldClass} /></label>
            </div>
          </div>
        </div>
        <div className="border-t border-slate-100/80 px-6 py-4 flex items-center justify-between gap-3 shrink-0 bg-slate-50/30">
