...
function TicketDrawer({ ticket, onClose, onChange, onSave, onDelete, onAddComment, staffOptions, currentUser, nowMs }) {
  ...
  useEffect(function () { setCommentText(""); }, [ticket ? ticket.id : ""]);
    ...
    <div className="fixed inset-0 z-40 flex">
      ...
      <div className="relative ml-auto flex h-full w-full max-w-2xl flex-col bg-white shadow-2xl overflow-hidden">
        ...
        <div className="flex items-start justify-between border-b border-slate-100 px-6 py-5 shrink-0">
          ...
          <button type="button" onClick={onClose} className="rounded-xl border border-slate-200 p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-50 transition shrink-0">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <label className="text-sm sm:col-span-2"><div className="mb-1.5 font-medium text-slate-600">Issue</div><input value={ticket.issue || ""} onChange={(e) => onChange(ticket.id, { issue: e.target.value })} className={fieldClass} /></label>
            <label className="text-sm"><div className="mb-1.5 font-medium text-slate-600">Received</div><input value={ticket.timestampReceived || ""} onChange={(e) => onChange(ticket.id, { timestampReceived: e.target.value })} className={fieldClass} /></label>
            ...
            <label className="text-sm"><div className="mb-1.5 font-medium text-slate-600">Responsible Person</div><input list="staff-opts" value={ticket.responsiblePerson || ""} onChange={(e) => onChange(ticket.id, { responsiblePerson: e.target.value })} className={fieldClass} /></label>
            <label className="text-sm"><div className="mb-1.5 font-medium text-slate-600">Escalated to</div><input list="staff-opts" value={ticket.escalatedTo || ""} onChange={(e) => onChange(ticket.id, { escalatedTo: e.target.value })} className={fieldClass} /></label>
            <label className="text-sm sm:col-span-2"><div className="mb-1.5 font-medium text-slate-600">Solution</div><input value={ticket.solution || ""} onChange={(e) => onChange(ticket.id, { solution: e.target.value })} className={fieldClass} /></label>
            <label className="text-sm"><div className="mb-1.5 font-medium text-slate-600">Status</div><select value={ticket.status} onChange={(e) => onChange(ticket.id, { status: e.target.value })} className={fieldClass}>{STATUS_ORDER.map(function (s) { return <option key={s} value={s}>{s}</option>; })}</select></label>
            <label className="text-sm"><div className="mb-1.5 font-medium text-slate-600">Solved At</div><input value={ticket.timestampSolved || ""} onChange={(e) => onChange(ticket.id, { timestampSolved: e.target.value })} className={fieldClass} /></label>
            <label className="text-sm sm:col-span-2"><div className="mb-1.5 font-medium text-slate-600">Duration</div><input value={ticket.importedDurationLabel || ""} onChange={(e) => onChange(ticket.id, { importedDurationLabel: e.target.value, duration: e.target.value })} placeholder="e.g. 3 days, Same day" className={fieldClass} /></label>
          </div>
        ...
        <div className="border-t border-slate-100 px-6 py-4 flex items-center justify-between gap-3 shrink-0">
          ...
          <button type="button" onClick={function () { onSave(ticket.id); onClose(); }} className="rounded-xl bg-gradient-to-r from-indigo-500 to-violet-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:shadow-md hover:brightness-110 transition-all">Save to Spreadsheet</button>
        </div>
