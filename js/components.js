...
function CreateTicketModal({ open, onClose, onCreate, staffOptions }) {
  const [form, setForm] = useState({ riderNo: "", riderName: "", issue: "", acknowledged: "", responsiblePerson: "", escalatedTo: "", solution: "", status: "Open" });
  useEffect(() => { if (!open) setForm({ riderNo: "", riderName: "", issue: "", acknowledged: "", responsiblePerson: "", escalatedTo: "", solution: "", status: "Open" }); }, [open]);
  if (!open) return null;
